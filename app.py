...
def to_year_series(cl_triangle_attr):
    ...
    Drops any all-NaN phantom rows chainladder occasionally appends."""
...
def wide_df_to_triangle(df):
    ...
    Expected layout:
      - Rows   = accident years  (e.g. 2018, 2019 … as index)
      - Columns = development periods in months (e.g. 12, 24, 36 …)
      - Values  = cumulative paid losses (NaN for future cells)

    Works regardless of whether column headers are integers, floats,
    or strings like '12', '24.0', etc.
    """
    from dateutil.relativedelta import relativedelta

    df = df.copy()

    # ── Normalise index: extract 4-digit year ─────────────────────────────
    try:
        df.index = (
        ...
        raise ValueError(
            "Row index must contain 4-digit accident years (e.g. 2018, 2019)."
        )

    # ── Normalise columns: strip decimals/spaces, convert to int ─────────
    try:
        df.columns = (
    ...
    # ── Melt to long form, drop NaN cells ────────────────────────────────
    df_long = (
        df.reset_index()
    ...
    # ── Convert origin year → Jan-1 date ─────────────────────────────────
    df_long["origin"] = pd.to_datetime(
        df_long["origin"].astype(str) + "-01-01"
    )
